<template>
    <div class="bg-principal pt-6">
        <main class="w-11/12 m-auto pb-6">
            ...
            <h1 class="md:w-3/5 -mt-20 text-4xl font-principal">
                    <mark class="bg-text">
                       {{ post.titulo }}
                    </mark>
            </h1>
            <h2 class="text-text/50 text-2xl font-secundaria ml-4 my-6">{{ post.tag }}</h2>
            <div class="md:w-3/5">
...
export default {
  ...
  async created() {
    ...
    const response = await axios.get(`https://d0xguy57.directus.app/items/Post${this.$route.path}`);
